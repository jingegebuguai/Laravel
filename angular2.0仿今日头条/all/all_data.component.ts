...
@Component({
  selector:'all_data',
  styleUrls:['all_data.component.css'],
  template:`
  <div *ngIf="image_length>0" class="hot">
    <a id="images" [routerLink]="['/article',urlSlice(),display_url_Slice(),urlSlice()]">
      <div class="title">
        <span>{{title}}</span>
      </div>
      <div class="images">
  ...
  <div *ngIf="image_length==0&&(middle_image_1||middle_image_2)" class="hot2">
    <a id="large_image" [routerLink]="['/article',urlSlice(),display_url_Slice(),urlSlice()]">
      <div class="left">
        <div class="title">
          <span>{{title}}</span>
        </div>
    ...
    */
    ...
    //获取推荐新闻数据
      ...
      this.toutiaoApiservice.searchAll().map(res => {
        if (res.json().data[5].article_genre == "article")
          return res.json().data[5]
      })
        .subscribe(response => (console.log(response), this.title = response.title, this.comments = response.comments_count,
          ...
          this.display_url=response.display_url))
